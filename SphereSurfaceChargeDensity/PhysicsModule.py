...
def sigma(R, V, epsilon_0 = 8.854*(10**(-12))):
    ...
    Parameters:
    R (float), V (float), epsilon_0 (float): input numbers (radius, potential, electric permitivity of free space)
    Returns:
    number (float): the surface charge density
    """
    return lambda epsilon_0, V, R: mult(epsilon_0, div(V, R))
