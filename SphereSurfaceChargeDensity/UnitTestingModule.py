...
def test_sigma():
    ...
    calculated_results = map(sigma, radii, potentials) # the map function
    expected_results = [Epsilon_0 * (potentials[i] / radii[i]) for i in range(len(potentials))] # List-comprehensions
